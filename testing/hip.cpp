...
void initCamera( void ) {
    ///*
    camera.position = Vector3( 2.0, 1.0, 2.0 );
    camera.viewpoint = Vector3( 0.0,-1.0, 0.0 );
...
void initBody( void ) {
    ...
    articulatedbody.append_link( left_leg );
    articulatedbody.append_link( right_leg );
...
void interpolate( Joint* joint ) {
    ...
    if( joint->trajectory.keyframes() ) {
        ...
        if( joint->trajectory.reversible ) {
            ...
            if( joint->trajectory.frame_tic == joint->trajectory.keyframe( joint->trajectory.keyframes() - 1 ).frame ) {
                ...
                joint->trajectory.forward = false;
            } else if ( joint->trajectory.frame_tic == joint->trajectory.keyframe( 0 ).frame && frame_id > joint->trajectory.keyframe( 0 ).frame ) {
                int temp = joint->trajectory.prev_keyframe_id;
                joint->trajectory.prev_keyframe_id = joint->trajectory.next_keyframe_id;
                joint->trajectory.next_keyframe_id = temp;
                joint->trajectory.forward = true;
            }
        } else {
            // if cyclic but not reversible and it has reached the end of the cycle, restart it
            if( joint->trajectory.frame_tic == joint->trajectory.keyframe( joint->trajectory.keyframes() - 1 ).frame ) {
                //joint->trajectory_position = 0.0;
                joint->trajectory.prev_keyframe_id = 0;
                joint->trajectory.next_keyframe_id = 1;
                joint->trajectory.frame_tic = 0;
            }
        }
        ...
        // if its transition point, simply use the pose at that keyframe
        // otherwise euler integrate the joint angle
        if( transition ) {
            joint->angle = keyframe0.pose.orientation;
        } else {
...
void interpolate_hierarchy( void ) {
    //interpolate_link( articulatedbody.root_link );
...
void display(void)
{
    ...
    //draw_trajectory( articulatedbody.root_link->trajectory, NULL );
    draw_articulatedbody( articulatedbody );

    Joint* left_hip = articulatedbody.root_link->child_joint( 0 );
    Joint* right_hip = articulatedbody.root_link->child_joint( 1 );

    Matrix4 F1, X1, J1, F2;
    F1 = left_hip->inboard_link->pose.transform;
    X1 = Matrix4::translationMatrix( left_hip->inboard_displacement );
    J1 = left_hip->frame_transformation * EulerAngle::matrix4( EULER_ANGLE_XYZ, EULER_HANDED_RIGHT, left_hip->median_angle );
    F2 = F1 * X1 * J1;
    draw_trajectory( left_hip->trajectory, &F2 );

    F1 = right_hip->inboard_link->pose.transform;
    X1 = Matrix4::translationMatrix( right_hip->inboard_displacement );
    J1 = right_hip->frame_transformation * EulerAngle::matrix4( EULER_ANGLE_XYZ, EULER_HANDED_RIGHT, right_hip->median_angle );
    F2 = F1 * X1 * J1;
    draw_trajectory( right_hip->trajectory, &F2 );
